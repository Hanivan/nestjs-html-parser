...
let HtmlParserService = HtmlParserService_1 = class HtmlParserService {
    ...
    async fetchHtml(url, options) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (config.verbose) {
                    this.logger.debug(`✅ Successfully fetched ${url} (${response.status} ${response.statusText})`);
                }
                return {
                    data: response.data,
                ...
                if (config.verbose) {
                    this.logger.error(`❌ Attempt ${attempt + 1} failed: ${errorInfo.type} - ${lastError.message}`);
                }
                // Check if we should retry based on error type
                const shouldRetry = this.shouldRetryOnError(errorInfo, config);
                if (config.verbose) {
                    this.logger.debug(`🤔 Should retry: ${shouldRetry}, Attempts left: ${maxRetries - attempt}`);
                }
    ...
    extractSingle(html, selector, type = 'xpath', attribute, options) {
        ...
        if (verbose) {
            this.logger.debug(`🔍 extractSingle - Selector: "${selector}", Type: ${type}, Attribute: ${attribute || 'none'}`);
        }
        try {
            let result;
            if (type === 'xpath') {
            ...
            if (verbose) {
                this.logger.debug(`✅ extractSingle result: ${result ? `"${result.substring(0, 100)}${result.length > 100 ? '...' : ''}"` : 'null'}`);
            }
    ...
    extractMultiple(html, selector, type = 'xpath', attribute, options) {
        const verbose = options?.verbose ?? this.defaultOptions.verbose ?? false;
        if (verbose) {
            this.logger.debug(`🔍 extractMultiple - Selector: "${selector}", Type: ${type}, Attribute: ${attribute || 'none'}`);
        }
        try {
            let results;
            if (type === 'xpath') {
            ...
            if (verbose) {
                ...
                if (results.length > 0 && results.length <= 5) {
                    results.forEach((result, index) => {
                        this.logger.debug(`   ${index + 1}: "${result.substring(0, 80)}${result.length > 80 ? '...' : ''}"`);
                    });
                }
    ...
    exists(html, selector, type = 'xpath', options) {
        const verbose = options?.verbose ?? this.defaultOptions.verbose ?? false;
        if (verbose) {
            this.logger.debug(`🔍 exists - Checking selector: "${selector}", Type: ${type}`);
        }
        try {
            let result;
            if (type === 'xpath') {
            ...
            if (verbose) {
                this.logger.debug(`✅ exists result: ${result ? 'Found' : 'Not found'}`);
            }
            return result;
        }
        catch (error) {
    ...
    count(html, selector, type = 'xpath', options) {
        const verbose = options?.verbose ?? this.defaultOptions.verbose ?? false;
        if (verbose) {
            this.logger.debug(`🔍 count - Counting selector: "${selector}", Type: ${type}`);
        }
        try {
            let result;
            if (type === 'xpath') {
            ...
            for (const [key, config] of Object.entries(schema)) {
                try {
                    ...
                    if (config.type === 'xpath') {
                        value = this.extractSingleXPath(html, config.selector, config.attribute, verbose);
                    }
                    else {
     ...
     * ```
     */
    extractStructuredList(html, containerSelector, schema, containerType = 'xpath', options) {
        const verbose = options?.verbose ?? this.defaultOptions.verbose ?? false;
        const results = [];
        if (verbose) {
            this.logger.debug(`🔍 extractStructuredList - Container: "${containerSelector}", Type: ${containerType}`);
        ...
        try {
            ...
            if (verbose) {
                this.logger.debug(`📦 Found ${containers.length} containers to process`);
            }
            for (let i = 0; i < containers.length; i++) {
                ...
                if (verbose) {
                    const extractedFields = Object.entries(item)
                        ...
                        .map(([key, _]) => key);
                    this.logger.debug(`✅ Container ${i + 1} extracted fields: ${extractedFields.join(', ')}`);
                }
            }
            if (verbose) {
     ...
     */
    evaluateXPath(html, xpath, verbose = false) {
        try {
            ...
            if (!verbose) {
                ...
                virtualConsole.on('error', (error) => {
                    ...
                    const isCSSError = errorString.includes('Could not parse CSS stylesheet') ||
                ...
                virtualConsole.on('warn', (warning) => {
                    ...
                    const isCSSWarning = warningString.includes('CSS') ||
                ...
                virtualConsole.on('info', (...args) => {
                    if (verbose)
                        this.logger.debug('XPath info:', ...args);
                });
                virtualConsole.on('log', (...args) => {
                    if (verbose)
                        this.logger.debug('XPath log:', ...args);
                });
            }
            ...
            const result = document.evaluate(xpath, document, null, dom.window.XPathResult.ANY_TYPE, null);
            const nodes = [];
            ...
            if (verbose && nodes.length === 0) {
                ...
                this.logger.debug(`XPath query "${xpath}" returned ${nodes.length} results`);
            }
            return nodes;
        }
        catch (error) {
    ...
    extractSingleXPath(html, selector, attribute, verbose = false) {
        try {
    ...
    extractMultipleXPath(html, selector, attribute, verbose = false) {
        try {
            const results = [];
            // Handle attribute selectors differently
            if (attribute && !selector.includes('/@')) {
    ...
    extractSingleCSS(html, selector, attribute) {
        try {
            const $ = cheerio.load(html);
    ...
    extractMultipleCSS(html, selector, attribute) {
        try {
            const $ = cheerio.load(html);
    ...
    categorizeError(error) {
        ...
        if (message.includes('self signed certificate') ||
            ...
            return {
                type: 'ssl',
                description: 'SSL certificate error - try setting rejectUnauthorized: false or ignoreSSLErrors: true',
            };
        }
        ...
        if (message.includes('timeout') || message.includes('etimedout')) {
            return {
                type: 'timeout',
                description: 'Connection timeout - try increasing timeout value or retry delay',
            };
        }
        ...
        if (message.includes('enotfound') || message.includes('getaddrinfo')) {
            return {
                type: 'dns',
                description: 'DNS resolution failed - domain might be dead or unreachable',
            };
        }
        ...
        if (message.includes('econnrefused') ||
            ...
            return {
                ...
                description: 'Connection refused - server might be down or blocking requests',
            };
        }
        // Network unreachable
        if (message.includes('enetunreach') ||
            ...
            return {
                ...
                description: 'Network unreachable - check internet connection or proxy settings',
            };
        }
        // Socket hang up
        if (message.includes('socket hang up') || message.includes('econnreset')) {
            return {
                ...
                description: 'Connection reset by server - try with different user agent or proxy',
            };
        }
        ...
        if (message.includes('429') || message.includes('rate limit')) {
            return {
                type: 'rateLimited',
                description: 'Rate limited - increase retry delay or use proxy rotation',
            };
        }
        // HTTP errors
     ...
     */
    shouldRetryOnError(errorInfo, config) {
        const retryConfig = config.retryOnErrors || this.defaultOptions.retryOnErrors || {};
...
exports.HtmlParserService = HtmlParserService = HtmlParserService_1 = __decorate([
    ...
    __param(0, (0, common_1.Inject)(html_parser_config_1.HTML_PARSER_LOGGER_LEVEL)),
